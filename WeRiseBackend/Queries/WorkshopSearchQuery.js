...
const searchWorkshops = async (req, res) => {
    ...
    if(req.body.search && req.body.categories && req.body.startDate){
        try {
            ...
            (created_workshops.start_time >= $2 AND created_workshops.end_time <= $3) ORDER BY created_workshops.start_time`,
    ...
    if(!req.body.categories.length && !req.body.search){
        try {
            let search = await database.any(
            `SELECT * FROM created_workshops JOIN users ON created_workshops.user_id = users.id
            WHERE created_workshops.start_time >= $1 AND created_workshops.end_time <= $2 ORDER BY created_workshops.start_time`,
